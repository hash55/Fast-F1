...
class Cache:
    ...
    def api_request_wrapper(cls, api_func):
        ...
        def cached_api_request(api_path, response=None, livedata=None):
            """Return cached data or download data if no cached data is
            available or if the cached data needs to be updated.
            Downloaded data will be added to the cache."""
            if cls._CACHE_DIR:
                # caching is enabled
                func_name = str(api_func.__name__)
                ...
                return api_func(api_path, response=response, livedata=livedata)

        wrapped = cached_api_request
        wrapped.__doc__ = api_func.__doc__  # necessary to make docstrings work
